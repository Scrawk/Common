...
namespace Common.Mathematics.Test.Functions
{
    ...
    public class QuotientFuncTest
    {
        ...
        public void Derivative()
        {
            ...
            Assert.AreEqual("g(2x * 1 + 5x - x^2 * 5) / h(1 + 5x * 1 + 5x)", derivative.ToString());
            ...
            Assert.AreEqual("g(-0.4e^-0.4x * x + x^2 - e^-0.4x * 1 + 2x) / h(x + x^2 * x + x^2)", derivative.ToString());
            ...
            Assert.AreEqual("g(PIcos(PIx) * x^2 - sin(PIx) * 2x) / h(x^2 * x^2)", derivative.ToString());
