...
namespace Common.Geometry.Nurbs
{
	...
	internal static class NurbsModify
	{
		...
		internal static RationalNurbsSurface3d RationalSurfaceKnotInsertU(RationalNurbsSurface3d srf, double u, int repeat = 1)
		{
			...
			List<double> knots_u;
			Vector4d[,] new_Cw;
			SurfaceKnotInsert(srf.DegreeU, srf.KnotsU, Cw, u, repeat, true, out knots_u, out new_Cw);

			width = new_Cw.GetLength(0);
			height = new_Cw.GetLength(1);

			// Convert back to cartesian coordinates
			var control_points = new Vector3d[width, height];
			var weights = new double[width, height];

			for (int i = 0; i < width; ++i)
			{
				for (int j = 0; j < height; ++j)
				{
					control_points[i, j] = NurbsUtil.HomogenousToCartesian(new_Cw[i, j]);
					weights[i, j] = new_Cw[i, j].w;
				}
			}
			...
			return new RationalNurbsSurface3d(degree_u, degree_v, knots_u, knots_v, control_points, weights);
		}
		...
		internal static RationalNurbsSurface3d RationalSurfaceKnotInsertV(RationalNurbsSurface3d srf, double v, int repeat = 1)
		{
			...
			List<double> knots_v;
			Vector4d[,] new_Cw;
			SurfaceKnotInsert(srf.DegreeV, srf.KnotsV, Cw, v, repeat, false, out knots_v, out new_Cw);
		...
		internal static (NurbsSurface3d, NurbsSurface3d) SurfaceSplitU(NurbsSurface3d srf, double u)
		{
			...
			Vector3d[,] left_controlPoints, right_controlPoints;
		...
		internal static (RationalNurbsSurface3d, RationalNurbsSurface3d) RationalSurfaceSplitU(RationalNurbsSurface3d srf, double u)
		{
			...
			SurfaceSplit(srf.DegreeU, srf.KnotsU, Cw, u, true,
				out left_knots_u, out left_Cw, out right_knots_u, out right_Cw);

			Vector3d[,] left_controlPoints;
			Vector3d[,] right_controlPoints;

			double[,] left_weights;
			double[,] right_weights;

			// Convert back to cartesian coordinates
			NurbsUtil.HomogenousToCartesian(left_Cw, out left_controlPoints, out left_weights);
			NurbsUtil.HomogenousToCartesian(right_Cw, out right_controlPoints, out right_weights);
			...
			var left = new RationalNurbsSurface3d(degreeU, degreeV, left_knots_u, left_knots_v, left_controlPoints, left_weights);
		...
		internal static (RationalNurbsSurface3d, RationalNurbsSurface3d) RationalSurfaceSplitV(RationalNurbsSurface3d srf, double v)
		{
			...
			SurfaceSplit(srf.DegreeV, srf.KnotsV, Cw, v, false,
		...
		internal static void SurfaceKnotInsert(int degree, IList<double> knots,
			Vector3d[,] cp, double knot, int r, bool along_u,
			out List<double> new_knots, out Vector3d[,] new_cp)
		{
			...
			var tmp = new List<Vector3d>(degree + 1);
			...
			if (along_u)
			{
				...
				new_cp = new Vector3d[width + r, height];
			...
			else
			{
				...
				new_cp = new Vector3d[width, height + r];
		...
		internal static void SurfaceSplit(int degree, IList<double> knots, Vector3d[,] control_points, double u, bool along_u,
			out List<double> left_knots, out Vector3d[,] left_control_points,
			out List<double> right_knots, out Vector3d[,] right_control_points)
		{
			List<double> tmp_knots;
			Vector3d[,] tmp_cp;
			...
			{
				int ii = 0;
				left_control_points = new Vector3d[ks + r, height];
				...
				ii = 0;
				right_control_points = new Vector3d[width - ks - r + 1, height];
			...
			{
				int ii = 0;
				left_control_points = new Vector3d[width, ks + r];
				...
				ii = 0;
				right_control_points = new Vector3d[width, height - ks - r + 1];
