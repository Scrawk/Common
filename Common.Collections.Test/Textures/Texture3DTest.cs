...
namespace Common.Collections.Test.Textures
{
    ...
    public class Collections_Textures_Texture3DTest
    {
        ...
        public void  GetChannels_DoesWrap()
        {
            ...
            tex.GetChannels(-1, 0, 0, tmp);
            Assert.AreEqual(1, tmp[0]);

            tex.GetChannels(2, 0, 0, tmp);
            Assert.AreEqual(0, tmp[0]);

            tex.GetChannels(0, -1, 0, tmp);
            Assert.AreEqual(1, tmp[1]);

            tex.GetChannels(0, 2, 0, tmp);
            Assert.AreEqual(0, tmp[1]);

            tex.GetChannels(0, 0, -1, tmp);
            Assert.AreEqual(1, tmp[2]);

            tex.GetChannels(0, 0, 2, tmp);
            Assert.AreEqual(0, tmp[2]);
