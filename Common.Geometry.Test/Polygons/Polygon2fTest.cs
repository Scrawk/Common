...
namespace Common.Geometry.Test.Polygons
{
    ...
    public class Polygon2fTest
    {
        ...
        public void GetPosition()
        {
            ...
            Assert.IsTrue(polygon.GetPosition(0.0f).EqualsWithError(new Vector2f(1, 2), eps));
            Assert.IsTrue(polygon.GetPosition(1.0f).EqualsWithError(new Vector2f(1, 2), eps));
            Assert.IsTrue(polygon.GetPosition(3.0f / len).EqualsWithError(new Vector2f(2, 0), eps));
            Assert.IsTrue(polygon.GetPosition(5.2f / len).EqualsWithError(new Vector2f(1, -1.2), eps));
            Assert.IsTrue(polygon.GetPosition(9.8f / len).EqualsWithError(new Vector2f(-1.8, -1), eps));
        }
